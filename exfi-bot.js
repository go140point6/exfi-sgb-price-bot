...
async function getInitialPrice() {
  ...
  const wsb = "wsb"
  const exfi_ADDRESS = "0xc348f894d0e939fe72c467156e6d7dcbd6f16e21"
  const exfi = "exfi"
  ...
  let exfiSGBPrice = (Number(arryPrice[1]._hex) / 10 ** 18).toFixed(5)
  let wsbUSDPrice = (sgbPrice / wsbSGBPrice).toFixed(5)
  let exfiUSDPrice = (sgbPrice / exfiSGBPrice).toFixed(5)
  console.log(wsb + " USD price is " + wsbUSDPrice)
  console.log(exfi + " USD price is " + exfiUSDPrice)

  clearRoles()
  lastPrice = exfiUSDPrice || 0
  let symbol = `${process.env.COIN_ID.toUpperCase()}`
  client.user.setPresence({
    activities: [{
    name: `SGB=${exfiSGBPrice} ${symbol}`,
    type: `PLAYING`
    }]
  })

  arrow = mid
  client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`${symbol} ${arrow} ${process.env.CURRENCY_SYMBOL}${exfiUSDPrice}`)
